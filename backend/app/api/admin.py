...
def dashboard(db: Session = Depends(get_db), _=Depends(admin_user)):
    ...
    return {
        ...
        "total_products": db.query(Product).filter_by(status=True).count(),
        "pending_orders": db.query(Order)
        .filter(Order.order_status == OrderStatus.PENDING)
        .count(),
        "review_analytics": {
            "average_store_rating": (
                round(float(review_average), 1) if review_average else 0
            ),
            "total_reviews": db.query(Review).count(),
            "verified_reviews": db.query(Review)
            .filter_by(verified_purchase=True)
            .count(),
            "pending_reviews": db.query(Review)
            .filter_by(status=ReviewStatus.PENDING)
            .count(),
            "reported_reviews": db.query(ReviewReport).filter_by(status="OPEN").count(),
        },
        "low_stock": [
            {
                "id": v.id,
            ...
            .filter(ProductVariant.stock_quantity < 10)
            .limit(10)
        ],
    }
...
def set_status(
    ...
    try:
        o.order_status = OrderStatus(data.order_status)
    except ValueError:
...
@router.get("/reviews")
def reviews(
    status: str | None = None, db: Session = Depends(get_db), _=Depends(admin_user)
):
    q = db.query(Review).options(
        joinedload(Review.product), joinedload(Review.user), joinedload(Review.reports)
    )
    if status:
...
def approve_review(
    review_id: int, db: Session = Depends(get_db), _=Depends(admin_user)
):
    ...
    return [
        {
            ...
            "status": x.status,
            "reporter": x.user.name,
        }
        for x in db.query(ReviewReport)
        .options(joinedload(ReviewReport.user))
