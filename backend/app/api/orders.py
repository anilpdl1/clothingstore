...
@router.put("/profile")
def update_profile(
    data: ProfileIn, user: User = Depends(current_user), db: Session = Depends(get_db)
):
...
def change_password(
    data: PasswordIn, user: User = Depends(current_user), db: Session = Depends(get_db)
):
...
def create_address(
    data: AddressIn, user: User = Depends(current_user), db: Session = Depends(get_db)
):
    if data.is_default:
        db.query(Address).filter_by(user_id=user.id).update({"is_default": False})
    a = Address(user_id=user.id, **data.model_dump())
    db.add(a)
    db.commit()
    return {"id": a.id}
